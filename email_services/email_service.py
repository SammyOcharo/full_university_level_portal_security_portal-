...
def login_otp_email(email, otp):
    ...
    try:
        email_subject = 'Security Login OTP'
        SYSTEM_EMAIL = settings.EMAIL_HOST_USER
        to = email
        message = f'Dear Security personnel, use otp {otp} to complete your request to sign in.' 
...
def password_reset_otp_email(email, otp):
    ...
    try:
        ...
        SYSTEM_EMAIL = settings.EMAIL_HOST_USER
        to = email
        message = f'Dear Security Personnel, use otp {otp} to complete your request to change password.' 
